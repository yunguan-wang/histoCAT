function [c,Q,bestpartition,bestpartitionhierarchy] = LouvainfromBin_Windows( filename, numiters )
% LOUVAINFROMBIN_WINDOWS
%
...
%[curr_path, ~, ~] = fileparts(pwd);%mfilename('fullpath')
disp('In windows..');
path = which('LouvainfromBin_Windows');
curr_path = fileparts(path);
curr_path = [curr_path filesep];
ps = [curr_path 'Louvain_d' filesep];


filename = strrep( filename, '.bin', '' );
% begin
fprintf(1, 'MATLAB: calling convert:\n');
command = [ps 'convert.exe -i ' filename '.bin -o ' filename '_graph.bin -w ' filename '_graph.weights' ];
fprintf(1,'%s\n', command );
system( command );
...
for iter = 1:numiters
    ...
    command = [ps 'community.exe ' filename '_graph.bin -l -1 -v -w ' filename '_graph.weights > ' filename '.tree'];
    fprintf( 1, '%s\n', command );
    [~,r] = system( command );
    fprintf(1, '\n');
    try
    ...
    command = [ps 'hierarchy.exe ' filename '.tree' ];
    fprintf(1, '%s\n', command );
    [~,r] = system( command );
    fprintf(1, '\n' );
    ...
    for level = 1:num_levels
        ...
        command = [ps 'hierarchy.exe ' filename '.tree -l ' num2str( level ) ' > ' filename '.tmp' ];
        fprintf(1, '%s\n', command );
        system( command );
